...
export default class AcceptSkillViewTest extends AbstractAdventureTest {
	...
	protected static async rendersExpectedButtonsForWhomeverSent() {
		...
		for (const friend of this.friends) {
			const buttons =
				friend.inviteSender === 'me' ? ['withdraw'] : ['accept', 'decline']
			for (const id of buttons) {
				vcAssert.assertRowRendersButton(this.friendsListVc, friend.id, id)
			}

			const notRendered =
				friend.inviteSender === 'me' ? ['accept', 'decline'] : ['withdraw']
			for (const id of notRendered) {
				vcAssert.assertRowDoesNotRenderButton(this.friendsListVc, friend.id, id)
			}
		}
	}
